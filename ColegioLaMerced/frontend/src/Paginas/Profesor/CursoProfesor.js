...
function CursoProfesor() {
  ...
  const descargarNotasExcel = () => {
    if (!cursoSeleccionado || alumnosPorCurso.length === 0) {
      alert("Seleccione un curso con alumnos para descargar las notas.");
      return;
    }

    const dataParaExcel = alumnosPorCurso.map(alumno => {
      const fila = { "Alumno": alumno.nombre };
      examenes.forEach(examen => {
        fila[examen.replace('examen', 'Ex. ')] = alumno.notas[examen] ?? "-";
      });
      fila["Promedio"] = promedioAlumno(alumno.notas);
      return fila;
    });
      ...
      <div className="page-content">
        ...
        {cursoSeleccionado && (
          <div className="notas-details-card">
            ...
            <button className="btn btn-success mt-3" onClick={descargarNotasExcel}>Descargar Notas en Excel</button>
          </div>
        )}
      </div>
    </div>
