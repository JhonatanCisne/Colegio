...
import './EstilosBarraLateralAlumno.css'; // Asegúrate de que este archivo CSS exista y esté configurado
import logo from '../Imagenes/Escudo.png'; // Ruta de la imagen del logo

// --- Iconos para la barra lateral (SVGs para un rendimiento óptimo) ---
...
function BarraDeNavegacionLateral({ nombre }) {
    ...
    <>
      ...
      <aside className="sidebar">
        ...
        <div className="sidebar-footer">
          <div className="user-profile">
            <div className="user-avatar">{nombre ? nombre.charAt(0) : 'A'}</div>
            <span className="user-name">{nombre || 'Alumno'}</span>
          </div>
          {/* El enlace de escritorio ahora apunta a /login */}
      ...
      {isMobileMenuOpen && (
        <nav className="mobile-nav">
          <NavLink to="/inicio" className="nav-link" onClick={() => setIsMobileMenuOpen(false)}>Inicio</NavLink>
          <NavLink to="/cursos" className="nav-link" onClick={() => setIsMobileMenuOpen(false)}>Mis Cursos</NavLink>
          <NavLink to="/asistencia" className="nav-link" onClick={() => setIsMobileMenuOpen(false)}>Asistencia</NavLink>
          <NavLink to="/anuncios" className="nav-link" onClick={() => setIsMobileMenuOpen(false)}>Anuncios</NavLink>
          {/* El enlace móvil ahora apunta a /login */}
          <Link to="/login" className="nav-link logout" onClick={() => setIsMobileMenuOpen(false)}>Cerrar Sesión</Link>
        </nav>
      )}
    </>
  );
